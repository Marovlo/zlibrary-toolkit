...
`config.yaml` 的 `default_site` 是主站，`fallback_sites` 是按顺序排列的备用站点。
启动或线路恢复时，先在当前节点/近地区节点验证主站；近节点都失败后才验证备用站，
再必要时尝试远地区节点。请求成功后保持当前“节点 + 站点”组合，不因单次抖动频繁切换；
Web 健康监控沿用约 1 小时空闲复检，只有复检失败才重新选择。CLI 和 Web 都通过
`cli._ensure_access()`/`AccessState` 共享这套顺序，搜索缓存按站点区分，避免切站后复用旧域名链接。
