...
> **接手/新 agent 请先看这里**：当前有一个明确未拍定的开放问题——候选版本
> 质量排序（同一本书多条记录时该选哪条），见**第十七节**。其它章节记录的都是
> 已验证、已定案的实现；第十七节例外，是讨论中被搁置、等数据支撑的产品问题，
> 不要凭直觉直接改代码。
...
## 十七、【未决问题，交接给下一步开发】候选版本质量排序：当前实现只是侥幸对，尚未定案

**这是当前唯一一个明确"讨论中、未拍定"的设计问题，接手者应该优先看这一节，
不要被"代码在跑、测试通过"误导成"已经解决"。**

### 现状：`_rank_candidates()` 的排序键，以及它为什么不可靠

第十三节的候选回退机制解决的是"同一本书的某条记录文件已失效（204）"，回退顺序由
`cli.py::_rank_candidates()` 决定：

```python
def key(b):
    fmt = (b.format or "").lower()
    return (
        -b.match_score(query),    # 1. 标题匹配度
        pref.index(fmt) if fmt in pref else len(pref),   # 2. 格式偏好
   -b.rating,      # 3. 评分
    )
```

即"先按标题匹配度选，再按格式偏好，最后按评分"。**这个排序本身没有考虑`size`
（文件大小）**，而实测证明这可能选到质量差的版本却完全不会报错——204死链回退能兜底
"文件失效"，但兜不住"文件能下载、只是这个版本本身很差"（模糊扫描件、内容缺页、
排版损坏等），这种情况下载会**成功**，用户拿到手才发现书不对。

### 已知的真实案例（DK魔法百科）与由此引出的争论

同一本书搜到2 条记录：

| 记录 | 大小 | 标题匹配度 | 评分 | 能否下载 |
|---|---|---|---|---|
| DK魔法百科 (魔法、巫術與神祕史…) | 47.98 MB | 50（近似） | 5.0 | 可下载 |
| DK魔法百科 | 6.24 MB | 100（完全匹配） | 4.9 | 204死链 |

用户在手机上人工选的正是 47.98MB 那条（而不是标题完全匹配的那条），按`_rank_candidates`
的实际排序结果这次恰好也选中了 47.98MB——**但这是因为 6.24MB 那条正好是死链被
204 挡掉了，不是排序逻辑本身把它排到了前面**。如果 6.24MB 那条不是死链（能正常
下载），当前逻辑会先选中它，用户会拿到一本可能残缺的小文件，且**没有任何报错或
提示**。

由此讨论过两个信号该不该纳入排序，双方观点没有达成一致，**明确搁置，等用户
实测后再议**（当时被"站点搜索服务临时故障"打断，尚未恢复测试）：

- **助手观点**：同格式内文件越大，大概率越"完整"（全彩图鉴/带图书籍尤其明显），
  评分受"打分人数"影响，很多书 `rating=0`（无人评价）不能公平参与排序，大小几乎
  每条记录都有，覆盖面更广。
- **用户观点（反驳，未被验证，可能是对的）**：大小和评分**没有必然的正相关**，
  甚至可能是反的——小文件更可能是"原生排版、文字可复制"的干净 PDF/epub（文字层
  编码好，压缩率高），大文件反而可能是"扫描/拍照"生成的影印版（图片堆起来的
  PDF，体积大但文字模糊、不可复制、不可搜索）。

**双方都只有直觉，没有数据支撑**——这正是需要下一步用真实数据验证的地方。

### 给下一个 agent 的具体建议（未执行，仅是方向）

1. **先转化成可验证的问题，别急着改排序公式**。核心问题其实是"这个 PDF/epub 是
 原生文字排版，还是扫描图片拼出来的"，这个**可以不靠 size/rating 这类间接信号，
   直接检测**：下载后（或者如果站点详情页有预览，甚至下载前）用 `pypdf`/`pdfplumber`
   之类的库读取 PDF 的文字层——如果 `extract_text()` 在有实际内容的页面上返回空或
 接近空，基本可以判定是扫描版；epub 本身就是文字为主的格式，这个问题相对少见。
   这比猜"大文件=模糊"靠谱得多，是可以拿到确定性结论的信号。
2. **同一本书多条记录的场景需要攒够样本再下结论**，不要只用 DK 这一个案例定规则。
   建议找 5~10 本"确认有多条记录、且已知哪条是纸质完整版"的书，把
   `size`/`rating`/`match_score`/以及上面提到的"文字层检测结果"都记下来，看这几个
   信号跟"版本好坏"实际的相关性有多强，再决定排序权重，而不是像现在这样凭案例直觉定。
3. **不管最终选什么排序信号，都建议把"选择理由"打进日志**（哪怕只在 `-v` 模式），
   这样即使排序偶尔选错，用户至少能看到"为什么选了这本"，方便事后复盘调整，而不是
   像现在这样悄无声息地下载。
4. 之前讨论中还有两个连带问题一起搁置了，一并交接：
   - **格式优先级 vs 质量**：`format_preference` 目前是硬性优先（有 epub 就不看更大的
     pdf），但DK 这类全彩图鉴的 epub 排版经常是崩的，是否要给格式优先级加一个"质量
     下限"（比如 epub 明显小于同书 pdf 很多时放弃 epub）没有定论。
- **交互模式要不要显式标注"已探测确认可下载"**：204 探测本身不耗账号额度，
     交互模式列候选时可以顺带标注，值得做但优先级低于上面的核心问题。

### 当前状态：不是 bug，是有意搁置

再次强调：`_rank_candidates()` 现在的行为是**明确的、经过测试的**（见十三节验证
记录），不是没写完的半成品；这一节记录的是"排序权重是否要把 size 纳入、怎么纳入"
这个更深层的产品/质量问题，因为缺数据支撑，双方主动决定暂不动代码，等有真实测试
结果再回来改。下一步开发不应该在没有新数据的情况下直接凭感觉调整权重。
